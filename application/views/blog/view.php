...
<div class="blog">
    <div class="header blog_view_header flex">
        <div class="max-width flex">
            <h1 class="name"><?= $article->header ?></h1>
            <ul class="flex">
                <li class="mobile"><?= yii::$app->formatter->asDate($article->date, 'dd.MM.yy') ?></li>
                <li class="desktop"><?= yii::$app->formatter->asDate($article->date) ?></li>
                <li><?= $article->author->fio ?></li>
                <li><?= $article->type->title ?></li>
            </ul>
        </div>
    </div>

    <div class="blog_view_content max-width flex">
        <div class="left">
        ...
        <div class="right">
            <div class="author">
                ...
                <section class="popular">
                    <span class="selector">Статьи автора</span>

                    <?php foreach ($article->authorArticlies as $key => $art) : ?>
                        <a href="<?= Url::to(['/blog/view', 'id' => $art->id]) ?>" class="flex">
                            <img src="<?= Url::to([$art->img]) ?>" alt="<?= $art->header ?>">
                            <div>
                                <!-- <p>Lorem ipsum dolor si...</p> -->
                                <p><?= mb_strimwidth($art->header, 0, 25, "...") ?></p>
                                <span><?= yii::$app->formatter->asDate($art->date) ?></span>
                            </div>
                        </a>
                    <?php endforeach; ?>
